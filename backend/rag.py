...
# ========== ChromaDB Setup ==========
# Initialize ChromaDB with in-memory storage (no persistence for serverless)
...
def _populate_knowledge_base():
    """Pre-populate the knowledge base with medical reference documents"""
    ...
    for doc in MEDICAL_KNOWLEDGE_BASE:
        ...
        logger.info(f"Pre-populated knowledge base with '{filename}' ({len(chunks)} chunks)")
    
    print(f"Knowledge base initialized with {len(MEDICAL_KNOWLEDGE_BASE)} documents")
...
def delete_document(doc_id: str) -> dict:
    ...
    chunk_ids = uploaded_docs[doc_id]["chunk_ids"]
...
def clear_all_documents() -> dict:
    ...
    user_doc_ids = [doc_id for doc_id, info in uploaded_docs.items() if not info.get("is_builtin", False)]
